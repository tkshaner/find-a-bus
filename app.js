...
const corsProxies = [
  ...
  {
    buildUrl: (url) => `https://yacdn.org/proxy/${url}`,
    headers: {},
    sendRequestedWithHeader: false
  },
  {
...
function getProxyHeaders(proxyIndex = 0) {
  ...
  const shouldSendRequestedWith = proxy?.sendRequestedWithHeader;

  if (shouldSendRequestedWith === undefined || shouldSendRequestedWith) {
    ...
    if (isVehicleEndpoint) {
      for (let proxyIndex = 0; proxyIndex < corsProxies.length; proxyIndex += 1) {
        try {
          ...
          const response = await fetch(proxyUrl, { headers: getProxyHeaders(proxyIndex) });
          if (!response.ok) {
